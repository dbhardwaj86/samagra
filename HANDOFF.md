...
> - **▶ NEXT: Phase F (the heavy async LLM lanes — NotebookLM audio/slides, image-generation figures).** PRATHAM student twin remains **Phase G** (deferred). **⚠ OWNER follow-ups:** (1) curate `concept_aliases.json` for the 3 residual no-pointer concepts (polarisation [UK/US spelling-split], radioactivity, transistors — surfaced by name at `coverage-build`); (2) `concept_graph.db` is gitignored — run `samagra factory coverage-build` after pulling to materialize it; (3) (carried from D2) run the opt-in Samadhan live smoke once (`SAMAGRA_LIVE_LLM_SMOKE=1 ANTHROPIC_API_KEY=… python -m pytest tests/test_samadhan_live_smoke.py -v`).
>
> ---
>
